/*
 * Professional backend for the simplified Discord clone.
 *
 * This server provides a REST API and WebSocket gateway for a chat platform
 * featuring guilds (servers), categories, channels, direct messages, role‑based
 * permissions, basic moderation slash commands, E2EE message storage and
 * Cloudinary‑backed media uploads.  Users authenticate with email and
 * password, obtain a JWT, and connect via Socket.IO for realtime messaging.
 *
 * The file is self contained for readability: all models, middleware and
 * helpers live here.  In a production codebase you would split this into
 * separate modules.
 */
...
// MongoDB connection
const MONGODB_URI = process.env.MONGODB_URI || 'mongodb://localhost:27017/discord_clone';
...
// Multer storage to handle file uploads (temporary). We rely on Cloudinary for storage.
 ...
 * is owned by the platform admin.  The admin can ban users platform‑wide.
 */
async function ensureGlobalGuild() {
  ...
  if (!admin) {
    ...
    admin = await User.create({ email: 'ceosolace@example.com', username: 'CeoSolace', salt, passwordHash, isPlatformAdmin: true });
  }
  // Create or find the global guild
  let guild = await Guild.findOne({ name: 'Ceosolace Official' });
  if (!guild) {
    guild = await Guild.create({ name: 'Ceosolace Official', owner: admin._id, image: null });
...
app.post('/api/register', async (req, res) => {
  try {
    ...
    const user = await User.create({ email, username: username || email.split('@')[0], salt, passwordHash });
    // Create personal guild for user
    const personalGuild = await Guild.create({ name: `${user.username}'s Server`, owner: user._id });
...
app.post('/api/guilds/:guildId/categories', authMiddleware, async (req, res) => {
  ...
  const { name } = req.body;
  if (!name) return res.status(400).json({ error: 'Name required' });
  if (!(await hasPerm(req.userId, guildId, 'manage'))) return res.status(403).json({ error: 'Forbidden' });
...
app.post('/api/categories/:categoryId/channels', authMiddleware, async (req, res) => {
  ...
  if (!(await hasPerm(req.userId, category.guild, 'manage'))) return res.status(403).json({ error: 'Forbidden' });
  const key = crypto.randomBytes(32).toString('base64');
  const channel = await Channel.create({ name, guild: category.guild, category: categoryId, encryptionKey: key });
  res.json(channel);
});
...
app.post('/api/dms', authMiddleware, async (req, res) => {
  ...
  // find existing DM
  let channel = await Channel.findOne({ isDM: true, participants: { $all: [req.userId, other._id] } });
  if (!channel) {
    const key = crypto.randomBytes(32).toString('base64');
    channel = await Channel.create({ isDM: true, participants: [req.userId, other._id], encryptionKey: key });
  }
  res.json(channel);
});

// List DM channels for user
app.get('/api/dms', authMiddleware, async (req, res) => {
  const channels = await Channel.find({ isDM: true, participants: { $in: [req.userId] } });
  res.json(channels);
});

// Get messages for a channel (decrypted)
app.get('/api/channels/:channelId/messages', authMiddleware, async (req, res) => {
  ...
  // Permission check
  if (channel.isDM) {
    if (!channel.participants.some((p) => String(p) === String(req.userId))) return res.status(403).json({ error: 'Not a participant' });
  } else {
    if (!(await hasPerm(req.userId, channel.guild, 'read'))) return res.status(403).json({ error: 'Forbidden' });
  }
...
// Upload image (Cloudinary). Accepts multipart form-data field "file".
app.post('/api/upload', authMiddleware, upload.single('file'), async (req, res) => {
  try {
    if (!req.file) return res.status(400).json({ error: 'Missing file' });
    const uploadRes = await cloudinary.uploader.upload(req.file.path, { folder: 'discord-clone' });
    // cleanup local file
...
app.post('/api/user/settings', authMiddleware, upload.single('avatar'), async (req, res) => {
  try {
    ...
    await user.save();
    res.json({ message: 'Updated', user: { username: user.username, avatar: user.avatar } });
  } catch (err) {
    res.status(500).json({ error: 'Settings update failed' });
  }
});

// Update guild settings (name, image) – only owner
app.post('/api/guilds/:guildId/settings', authMiddleware, upload.single('image'), async (req, res) => {
  const { guildId } = req.params;
  const { name } = req.body;
  const guild = await Guild.findById(guildId);
  if (!guild) return res.status(404).json({ error: 'Guild not found' });
  if (String(guild.owner) !== String(req.userId)) return res.status(403).json({ error: 'Only owner can update settings' });
  if (name) guild.name = name;
  if (req.file) {
    const uploadRes = await cloudinary.uploader.upload(req.file.path, { folder: 'guilds' });
...
io.on('connection', (socket) => {
  ...
  socket.on('joinGuild', async (guildId) => {
    try {
      ...
      const channels = await Channel.find({ $or: [ { guild: guildId }, { category: { $in: catIds } } ] });
  ...
  socket.on('joinChannel', async (channelId) => {
    ...
    if (channel.isDM) {
      ...
      const bannedSet = banned.get(channel.guild);
  ...
  socket.on('sendMessage', async ({ channelId, content }) => {
    ...
    if (channel.isDM) {
      ...
      const mutedSet = muted.get(channel.guild);
    ...
    if (!channel.isDM && content.startsWith('/')) {
      await handleCommand(content, socket, channel);
      return;
    }
    const enc = encrypt(content, channel.encryptionKey);
    const msg = await Message.create({ channel: channelId, author: socket.userId, encrypted: enc.encrypted, iv: enc.iv, tag: enc.tag });
...
async function handleCommand(cmdStr, socket, channel) {
  ...
  switch (cmd) {
    case '/ban': {
      const email = parts[1];
      const user = await User.findOne({ email });
      if (!user) { socket.emit('errorMessage', 'User not found'); break; }
      ensureSet(banned, guildId.toString()).add(user._id.toString());
    ...
    case '/unban': {
      ...
      const user = await User.findOne({ email });
      if (!user) { socket.emit('errorMessage', 'User not found'); break; }
      const set = ensureSet(banned, guildId.toString());
      set.delete(user._id.toString());
    ...
    case '/mute': {
      ...
      const user = await User.findOne({ email });
      if (!user) { socket.emit('errorMessage', 'User not found'); break; }
      ensureSet(muted, guildId.toString()).add(user._id.toString());
    ...
    case '/unmute': {
      ...
      const user = await User.findOne({ email });
      if (!user) { socket.emit('errorMessage', 'User not found'); break; }
      const set = ensureSet(muted, guildId.toString());
      set.delete(user._id.toString());
    ...
    case '/role': {
      ...
      else { mem.role = newRole; await mem.save(); }
...
// Ensure global guild on startup
ensureGlobalGuild().then(() => {
  console.log('Global guild ready');
});
